...
export const NewsCard: React.FC<NewsCardProps> = ({ post, isThreadChild, hasThreadParent }) => {
  return (
    <div className="relative">
      ...
      {hasThreadParent && (
        <div className="absolute left-[2.4rem] -top-6 bottom-6 w-0.5 bg-neutral-800 -z-10" />
      )}

      {post.isRetweet && (
        <div className="flex items-center gap-2 text-neutral-500 text-xs font-bold mb-1.5 ml-2.5">
      ...
      <Card className={cn(
        ...
        isThreadChild 
          ? "bg-neutral-900/20 border-neutral-800/50 mt-2 shadow-none" 
          : "bg-neutral-900/60 border-neutral-800",
        "p-5"
      )} disableHover>
        <div className="flex gap-4">
          <div className="flex flex-col items-center shrink-0">
            <Avatar className="h-12 w-12 border-2 border-neutral-800 shadow-sm">
              <AvatarImage src={post.author.avatarUrl} alt={post.author.name} />
              <AvatarFallback>{post.author.name[0]}</AvatarFallback>
            </Avatar>
            {post.threadChildren && post.threadChildren.length > 0 && (
              <div className="w-0.5 flex-1 bg-neutral-800 mt-3 min-h-[20px]" />
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex flex-col">
                <div className="flex items-center gap-1.5">
                  <span className="font-bold text-neutral-100 text-[15px] hover:underline cursor-pointer">
                    {post.author.name}
                  </span>
              ...
              <button className="text-neutral-500 hover:text-white transition-colors p-1 rounded-md hover:bg-neutral-800">
                <MoreHorizontal className="w-4 h-4" />
              </button>
            </div>

            {post.content && (
              <div className="text-neutral-200 whitespace-pre-wrap mb-4 text-[15px] leading-relaxed font-normal">
            ...
            {post.media && post.media.length > 0 && (
              <div className={cn(
                "grid gap-0.5 mb-4 rounded-xl overflow-hidden border border-neutral-800/50",
                ...
                {post.media.map((media, index) => {
                  ...
                  return (
                    <div key={index} className={cn(
                      "relative bg-neutral-900 overflow-hidden cursor-pointer",
            ...
            <div className="flex items-center gap-6 text-neutral-500 pt-2 border-t border-neutral-800/50">
