...
import { TixIcon } from '@renderer/shared/ui/icons/TixIcon'
...
import { useTransactionTypes, useTransactions, useTransactionTotals } from './useTransactions'
import {
          ...
          <>
            {group.transactions.map((transaction, index) => (
              ...
              >
                <td className="py-2.5 px-4 text-sm text-neutral-400">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="relative flex items-center">
                      <div
                        className={`absolute left-0 w-0.5 bg-neutral-700 ${index === group.transactions.length - 1 ? 'h-1/2 top-0' : 'h-full'
                          }`}
                      />
                ...
                <td
                  className={`py-2.5 px-4 text-sm text-right font-mono ${isTickets
                    ? 'text-[#cc9e71]/80'
...
const TransactionsTab = ({ account }: TransactionsTabProps) => {
  ...
  const {
    ...
    error: typesError
  } = useTransactionTypes(cookie, userId)
  ...
  const {
    ...
    error: totalsError
  } = useTransactionTotals(cookie, timeFrame, userId)
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <EmptyState
          ...
          description="Choose an account with a valid cookie to load its transaction history."
        ...
        <PageHeaderPortal>
          {selectedType === 'all' && (
            <FloatingAction.Dropdown
              icon={Calendar}
              tooltip="Time Frame"
              options={TIME_FRAME_OPTIONS.map((o) => ({ value: o.value, label: o.label }))}
              value={timeFrame}
              onChange={(value) => setTimeFrame(value as TransactionTimeFrame)}
            />
          )}
        </PageHeaderPortal>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-5">
                <div className="flex items-center justify-between">
                  ...
                  <div
                    className={`text-2xl font-bold flex items-center gap-2 ${incomingTotal - outgoingTotal >= 0 ? 'text-emerald-400' : 'text-red-400'
                      }`}
                  >
                    <RobuxIcon className="w-5 h-5" />
