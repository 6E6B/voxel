...
import { useOnboardingStore, OnboardingStep } from '../useOnboardingStore'
              ...
              {STEPS.slice(1).map((step, index) => {
                ...
                return (
                  <React.Fragment key={step.id}>
                    <motion.div
                      className={`flex items-center justify-center w-8 h-8 rounded-full border-2 transition-colors ${isActive
                    ...
                    {index < STEPS.length - 2 && (
                      <div
                        className={`w-8 h-0.5 transition-colors ${isCompleted ? 'bg-emerald-500' : 'bg-neutral-800'
                          }`}
                      />
                    )}
                  </React.Fragment>
