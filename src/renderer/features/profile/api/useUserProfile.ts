...
import { z } from 'zod'
import { robloxBadgeSchema, usernameHistorySchema } from '@shared/contracts/user'
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (groupsData && groupsData.length > 0) {
        ...
        const thumbnails = await window.api.getBatchThumbnails(groupIds, 'GroupIcon')
...
export function useRobloxBadges(userId: number, cookie: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const badges = await robloxGet(z.array(robloxBadgeSchema), `https://accountinformation.roblox.com/v1/users/${userId}/roblox-badges`, { cookie })
      return (Array.isArray(badges) ? badges.slice(0, 12) : []).map((b) => ({
        id: b.id,
        name: b.name,
...
export function usePastUsernames(userId: number, cookie: string, enabled: boolean = false) {
  return useQuery({
    ...
    queryFn: async () => {
      const pastNamesData = await robloxGet(usernameHistorySchema, `https://users.roblox.com/v1/users/${userId}/username-history?limit=100&sortOrder=Desc`)
