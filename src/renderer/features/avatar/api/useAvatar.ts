...
import type { SuccessResponse } from '@shared/contracts/common'
...
export function useInventory(
  ...
  const userId = account?.userId ? parseInt(account.userId) : undefined
  const accountId = account?.id || ''
  const sortedAssetTypeIds = [...assetTypeIds].sort((a, b) => a - b)

  return useQuery({
    queryKey: queryKeys.avatar.inventory(accountId, sortedAssetTypeIds),
    queryFn: async (): Promise<InventoryItem[]> => {
      if (!cookie || !userId) return []

      const inventoryPromises = sortedAssetTypeIds.map((assetTypeId) =>
      ...
      const dedupedAssets = Array.from(
        new Map(allAssets.map((asset: any) => [asset.assetId, asset])).values()
      )

      const assetIds = dedupedAssets.map((i: any) => i.assetId)
      ...
      return dedupedAssets.map((asset: any) => ({
        id: asset.assetId,
    ...
    enabled: !!cookie && !!userId && sortedAssetTypeIds.length > 0 && (options?.enabled ?? true),
...
export function useSetWearingAssets(account: Account | null) {
  ...
  return useMutation<WearingAssetsResult, Error, AvatarAsset[]>({
...
export function useCreateOutfit(account: Account | null) {
  const queryClient = useQueryClient()
  const cookie = account?.cookie
  const accountId = account?.id || ''
  ...
  return useMutation({
    mutationFn: (name: string) => {
      if (!cookie) throw new Error('No cookie available')
      return window.api.createOutfit(cookie, name)
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.avatar.outfits(accountId, true) })
      queryClient.invalidateQueries({ queryKey: queryKeys.avatar.outfits(accountId, false) })
