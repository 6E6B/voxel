...
import { Dialog, DialogContent, DialogBody } from '@renderer/shared/ui/dialogs/Dialog'

// Inline Purchase Confirm Dialog
export const PurchaseConfirmDialog: React.FC<{
  isOpen: boolean
  onClose: () => void
  onConfirm: () => void
  assetName: string
        ...
        <DialogBody className="flex flex-col items-center text-center py-6">
          ...
          <div className="flex gap-3 w-full">
            ...
            <Button
              ...
              disabled={!canAfford}
              className={cn(
                'flex-1 text-white border shadow-none focus-visible:ring-0',
                !canAfford
                  ? 'bg-red-600 hover:bg-red-500 active:bg-red-700 border-red-500/60 cursor-not-allowed opacity-80'
                  : 'border-emerald-500/60 bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700'
              )}
            >
...
export const AssetPricing: React.FC<AssetPricingProps> = ({
  ...
  }, [cookie, userId, details.id, displayPrice, ownershipRefreshKey])
  ...
  return (
    <div className="p-4 bg-neutral-900/50 border border-neutral-800 rounded-xl flex items-center justify-between">
      ...
      <Button
        ...
        className={cn(
          'min-w-[120px] focus-visible:ring-0 shadow-none',
          isOwned
            ? 'bg-neutral-800 text-neutral-400 cursor-not-allowed hover:bg-neutral-800 active:bg-neutral-800 border-transparent'
            : displayPrice === 'Off Sale' || !canPurchase
              ? 'opacity-50 cursor-not-allowed bg-transparent text-neutral-400 border-2 border-dashed border-neutral-700 hover:bg-transparent active:bg-transparent'
              : 'bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white border-emerald-500/60'
        )}
      >
        {isPurchasing ? (
