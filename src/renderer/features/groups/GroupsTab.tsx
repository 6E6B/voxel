...
import { Users, Clock, ChevronRight, User, RefreshCw } from 'lucide-react'
...
const GroupItem = ({ group, role, thumbnail, isSelected, isPending, onClick }: GroupItemProps) => {
  return (
    ...
    >
      <button
        onClick={onClick}
        className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-left group overflow-hidden ${isSelected
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <span
              className={`font-medium truncate text-sm ${isSelected ? 'text-[var(--color-text-primary)]' : 'text-neutral-200'
                }`}
            >
              {group.name}
            </span>
      ...
      <div className="flex flex-col h-full bg-[var(--color-surface)]">
        ...
        {!selectedAccount ? (
          ...
          <div className="flex-1 flex min-h-0">
            ...
            <div
              ref={sidebarRef}
              className={`relative border-r border-neutral-800 flex flex-col shrink-0 bg-[var(--color-surface-strong)] ${!isResizing ? 'transition-[width] duration-150 ease-in-out' : ''}`}
