...
import { Settings, DEFAULT_ACCENT_COLOR } from '@renderer/shared/types'
import {
...
export function useSetAccountsViewMode() {
  ...
  return useMutation({
    ...
    onMutate: async (mode) => {
      await queryClient.cancelQueries({ queryKey: queryKeys.settings.accountsViewMode() })

      const previousMode = queryClient.getQueryData<'list' | 'grid'>(
        queryKeys.settings.accountsViewMode()
      )
    ...
    onError: (_error, _mode, context) => {
      if (context?.previousMode) {
        queryClient.setQueryData(queryKeys.settings.accountsViewMode(), context.previousMode)
      }
    },
