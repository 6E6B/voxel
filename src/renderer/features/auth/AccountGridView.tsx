...
import { timeAgo } from '@renderer/shared/utils/timeUtils'
...
const AccountGridView = ({
  ...
  const getAvatarRingClass = (status: AccountStatus) =>
    status !== AccountStatus.Offline ? `ring-4 ${getStatusRingUtilityClass(status)}` : ''
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col items-center text-center mt-2">
                <div>
                  <Avatar
                    className={`w-20 h-20 mb-4 shadow-lg border-0 privacy-blur ${getAvatarRingClass(account.status)}`}
                  >
                ...
                {voiceBanInfo?.[account.id] && (
                  <span className="text-xs text-red-400 text-center">{voiceBanInfo[account.id].message}</span>
                )}
