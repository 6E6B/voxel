...
import GenericContextMenu from '@renderer/shared/ui/menus/GenericContextMenu'
...
import {
  ...
  useDeployHistory,
  getApiType
} from './useInstallationsStore'
import type { DetectedInstallation } from '@shared/contracts/system'
...
const InstallTab: React.FC = () => {
  ...
  const handleCheckForUpdates = async (install: UnifiedInstallation) => {
    ...
    try {
      const result = await window.api.checkRobloxUpdates(getApiType(binaryType), install.version)
  ...
  return (
    <>
      <motion.div
        ...
        className="flex flex-col h-full bg-[var(--color-surface)] overflow-hidden text-[var(--color-text-secondary)]"
      >
        <InstallationsHeader
        ...
        <div className="flex-1 overflow-y-auto p-5 scrollbar-thin">
          <InstallationList
            ...
            onContextMenu={(e, install) => {
              setContextMenu({
                position: createAnchoredOverlayPosition(e),
                install
              })
            }}
            onNew={() => !isMac && setShowNewModal(true)}
