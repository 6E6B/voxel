...
import { Account, TabId } from '@renderer/shared/types'
...
import { useTabTransition } from '@renderer/shared/hooks/useTabTransition'
import { VoxelLogo } from '@renderer/shared/ui/icons/VoxelLogo'
import { getDropdownMotion } from '@renderer/shared/ui/menus/dropdownMotion'
import { SIDEBAR_TAB_DEFINITION_MAP, SidebarTabDefinition } from './sidebarTabs'
          ...
          <TooltipTrigger asChild>
            <div className="px-3 py-3" ref={containerRef}>
              ...
              <AnimatePresence>
                {isDropdownOpen && (
                  <motion.div
                    {...getDropdownMotion({ transformOrigin: 'bottom left', direction: 'up' })}
  ...
  return (
    <TooltipProvider>
      ...
      >
        ...
        <div
          className={`flex items-center shrink-0 bg-[var(--color-surface-strong)] transition-all duration-300 ${isSidebarCollapsed ? 'justify-center px-0' : 'justify-between pl-5 pr-3'
            }`}
          style={{
            ...
            {sidebarTabsToRender.map((tab, index) => {
              ...
              return (
                <React.Fragment key={tab.id}>
                  {showSeparator && (
                    <div className="my-2 mx-2.5 border-t border-[var(--color-border)]" />
                  )}
                  <SidebarItem
                    icon={tab.icon}
                    label={tab.label}
