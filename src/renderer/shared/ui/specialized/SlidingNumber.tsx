...
function SlidingNumber({
  ...
  const animatedValue = useSpring(targetValue, resolvedTransition)
  ...
  React.useEffect(() => {
    return animatedValue.on('change', (latest) => {
      if (Math.abs(latest - displayValue) > 0.001) return
      if (Math.abs(animatedValue.getVelocity()) > 0.01) return
      if (Math.abs(animatedValue.get() - displayValue) <= Number.EPSILON) return

      animatedValue.jump(displayValue)
    })
