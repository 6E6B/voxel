...
export const safeRequest = <T>(options: RequestOptions): Promise<T> => {
  return new Promise((resolve, reject) => {
    ...
    const isBodylessWriteRequest =
      options.body === undefined &&
      (normalizedMethod === 'POST' || normalizedMethod === 'PUT' || normalizedMethod === 'PATCH')
    const hasCustomContentType = hasContentTypeHeader(options.headers)
    ...
    const send = async () => {
      ...
      } else if (isBodylessWriteRequest && !hasCustomContentType) {
        request.setHeader('Content-Type', 'application/json')
...
export const safeRequestWithCsrf = async (options: RequestOptions): Promise<unknown> => {
  try {
