...
const normalizeLegacySidebarSettings = (rawData: unknown): { data: unknown; changed: boolean } => {
  if (!rawData || typeof rawData !== 'object') {
    return { data: rawData, changed: false }
  }

  const store = rawData as {
    settings?: {
      sidebarTabOrder?: unknown
      sidebarHiddenTabs?: unknown
    }
  }
  ...
  if (Array.isArray(store.settings.sidebarTabOrder)) {
    const normalizedOrder = sanitizeSidebarOrder(store.settings.sidebarTabOrder as TabId[])
    if (JSON.stringify(normalizedOrder) !== JSON.stringify(store.settings.sidebarTabOrder)) {
      nextSettings.sidebarTabOrder = normalizedOrder
      changed = true
    }
  }

  if (Array.isArray(store.settings.sidebarHiddenTabs)) {
    const normalizedHidden = sanitizeSidebarHidden(store.settings.sidebarHiddenTabs as TabId[])
    if (JSON.stringify(normalizedHidden) !== JSON.stringify(store.settings.sidebarHiddenTabs)) {
      nextSettings.sidebarHiddenTabs = normalizedHidden
      changed = true
    }
  }
...
class StorageService {
  ...
  private load(): void {
    try {
      ...
      const normalized = normalizeLegacySidebarSettings(rawData)
      const result = storeDataSchema.safeParse(normalized.data)
  ...
  public getSettings() {
    ...
    const LEGACY_DEFAULT_ACCENT_COLORS = ['#1e66f5', '#3b82f6', '#2563eb', '#e05c1a']
